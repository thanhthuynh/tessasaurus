//
//  TessaColors.swift
//  Tessasaurus
//

import SwiftUI

enum TessaColors {
    // Primary purple palette
    static let primary = Color("TessaPrimary")
    static let primaryLight = Color("PrimaryLight")
    static let primaryDark = Color(hex: "6D28D9")

    // Sunrise gradient colors
    static let pink = Color("TessaPink")
    static let coral = Color("Coral")
    static let orange = Color("TessaOrange")
    static let gold = Color("Gold")

    // Deep space colors
    static let deepSpace = Color(hex: "0D0D2B")
    static let nebula = Color(hex: "1A0A3E")
    static let starWhite = Color(hex: "E8E8FF")

    // Background colors
    static let cream = Color("Cream")
    static let background = Color("Background")

    // Semantic colors
    static let cardBackground = Color.white.opacity(0.2)
    static let cardBorder = Color.white.opacity(0.3)
    static let textPrimary = Color.white
    static let textSecondary = Color.white.opacity(0.8)
    static let textTertiary = Color.white.opacity(0.5)
    static let inputBackground = Color.white.opacity(0.1)
}

extension Color {
    init(hex: String) {
        let hex = hex.trimmingCharacters(in: CharacterSet.alphanumerics.inverted)
        var int: UInt64 = 0
        Scanner(string: hex).scanHexInt64(&int)
        let a, r, g, b: UInt64
        switch hex.count {
        case 3: // RGB (12-bit)
            (a, r, g, b) = (255, (int >> 8) * 17, (int >> 4 & 0xF) * 17, (int & 0xF) * 17)
        case 6: // RGB (24-bit)
            (a, r, g, b) = (255, int >> 16, int >> 8 & 0xFF, int & 0xFF)
        case 8: // ARGB (32-bit)
            (a, r, g, b) = (int >> 24, int >> 16 & 0xFF, int >> 8 & 0xFF, int & 0xFF)
        default:
            (a, r, g, b) = (1, 1, 1, 0)
        }
        self.init(
            .sRGB,
            red: Double(r) / 255,
            green: Double(g) / 255,
            blue: Double(b) / 255,
            opacity: Double(a) / 255
        )
    }
}
